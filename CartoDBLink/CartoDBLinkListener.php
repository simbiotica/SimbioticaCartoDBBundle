...
class CartoDBLinkListener extends MappedEventSubscriber
{
    ...
    public function onFlush(EventArgs $eventArgs)
    {
            ...
            if ($config = $this->getConfiguration($om, $meta->name)) {
                ...
                foreach ($ea->getObjectChangeSet($uow, $object) as $field => $changes) {
                    ...
                    if ($meta->isSingleValuedAssociation($field) && $value) {
                        ...
                        if ($config['columns'][$field]->set != '%s') {
                            $transformers[$config['columns'][$field]->column] = $config['columns'][$field]->set;
                        }
                    }
                }
                ...
                $connection->updateRow($config['table'], $cartodbid, $data, $transformers);
            }
        }
        // on insertion
        ...
        foreach ($ea->getScheduledObjectDeletions($uow) as $object) {
            ...
            if ($config = $this->getConfiguration($om, $meta->name)) {
                ...
                if ($cartodbid) {
                    $connection->deleteRow($config['table'], $cartodbid);
                }
            }
        }
    }
    ...
    public function postPersist(EventArgs $args)
    {
        ...
        if ($this->pendingInserts && array_key_exists($oid, $this->pendingInserts)) {
            ...
            $connection->updateRow($config['table'], $this->pendingInserts[$oid], array($index => $id));
        ...
        if ($this->pendingRelatedObjects && array_key_exists($oid, $this->pendingRelatedObjects)) {
            ...
            foreach ($this->pendingRelatedObjects[$oid] as $configAssoc) {
                ...
                $connection->updateRow(
                    $configAssoc['table'],
                    $configAssoc['cartodbid'],
                    array($configAssoc['field'] => $cartodbid)
                );
            }
    ...
    public function postLoad(EventArgs $args)
    {
        ...
        $object = $ea->getObject();
        $meta = $om->getClassMetadata(get_class($object));
        $this->getConfiguration($om, $meta->name);
        if ($config = $this->getConfiguration($om, $meta->name)) {
